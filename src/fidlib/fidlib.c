...
//
//	This file depends on the fidmkf.c file which provides the
...
//
//	  LpBu4/20.4	Lowpass butterworth, 4th order, -3dB at 20.4Hz
...
//
//	The routine fid_design() is used to convert this spec-string
...
//
//	Support code (you may wish to replace the error() routine with
//	your own code to throw an exception or take some other
//	error-handling action rather than termination)
//
...
//	//
//	//	Convolve the given filter with itself to the given power.
//	//	Frees the old filter array, returns the new one.
//	//
//	
//	static double *
//	convolve(int pow, double *in) {
//	   int n_iir= (int)in[0];
//	   int m_iir= 1 + (n_iir - 1) * pow;
//	   int n_fir= (int)in[1];
//	   int m_fir= 1 + (n_fir - 1) * pow;
//	   double *rv= ALLOC_ARR(2 + m_iir + m_fir, double);
//	   double *in_iir= in+2;
//	   double *in_fir= in+2+n_iir;
//	   double *rv_iir= rv+2;
//	   double *rv_fir= rv+2+m_iir;
//	   int c_iir, c_fir;
//	
//	   rv[0]= m_iir;
//	   rv[1]= m_fir;
//	
//	   // Copy in initial filter
//	   memcpy(rv_iir, in_iir, n_iir * sizeof(double));
//	   memcpy(rv_fir, in_fir, n_fir * sizeof(double));
//	   c_iir= n_iir;
//	   c_fir= n_fir;
//	
//	   // Keep convolving until we meet the required power
//	   while (c_iir < m_iir) {
//	      int a, b;
//	
//	      // IIR first
//	      int len= c_iir + n_iir - 1;
//	      for (a= len-1; a>=0; a--) {
//		 double val= 0;
//		 for (b= 0; b<n_iir; b++)
//		    if (a-b >= 0 && a-b < c_iir)
//		       val += in_iir[b] * rv_iir[a-b];
//		 rv_iir[a]= val;
//	      }
//	      c_iir= len;
//	
...
//
//	Note that #O #F and #R are mapped to the f0/f1/order
...
static struct {
   ...
   { des_bpre, "BpRe/#V/#F",
     "Bandpass resonator, Q=#V (0=>Inf), frequency #F" },
   { des_bsre, "BsRe/#V/#F",
     "Bandstop resonator, Q=#V (0=>Inf), frequency #F" },
   { des_apre, "ApRe/#V/#F",
     "Allpass resonator, Q=#V (0=>Inf), frequency #F" },
   { des_pi, "Pi/#F",
     "Proportional-integral filter, frequency #F" },
   { des_piz, "PiZ/#F",
     "Proportional-integral filter, matched z-transform, frequency #F" },
   { des_lpbe, "LpBe#O/#F", 
     "Lowpass Bessel filter, order #O, -3dB frequency #F" },
   { des_hpbe, "HpBe#O/#F", 
     "Highpass Bessel filter, order #O, -3dB frequency #F" },
   { des_bpbe, "BpBe#O/#R", 
     "Bandpass Bessel filter, order #O, -3dB frequencies #R" },
   { des_bsbe, "BsBe#O/#R", 
     "Bandstop Bessel filter, order #O, -3dB frequencies #R" },
   { des_lpbu, "LpBu#O/#F", 
     "Lowpass Butterworth filter, order #O, -3dB frequency #F" },
   { des_hpbu, "HpBu#O/#F", 
     "Highpass Butterworth filter, order #O, -3dB frequency #F" },
   { des_bpbu, "BpBu#O/#R", 
     "Bandpass Butterworth filter, order #O, -3dB frequencies #R" },
   { des_bsbu, "BsBu#O/#R", 
     "Bandstop Butterworth filter, order #O, -3dB frequencies #R" },
   { des_lpch, "LpCh#O/#V/#F",
     "Lowpass Chebyshev filter, order #O, passband ripple #VdB, -3dB frequency #F" },
   { des_hpch, "HpCh#O/#V/#F", 
     "Highpass Chebyshev filter, order #O, passband ripple #VdB, -3dB frequency #F" },
   { des_bpch, "BpCh#O/#V/#R", 
     "Bandpass Chebyshev filter, order #O, passband ripple #VdB, -3dB frequencies #R" },
   { des_bsch, "BsCh#O/#V/#R", 
     "Bandstop Chebyshev filter, order #O, passband ripple #VdB, -3dB frequencies #R" },
   { des_lpbez, "LpBeZ#O/#F",
     "Lowpass Bessel filter, matched z-transform, order #O, -3dB frequency #F" },
   { des_hpbez, "HpBeZ#O/#F",
     "Highpass Bessel filter, matched z-transform, order #O, -3dB frequency #F" },
   { des_bpbez, "BpBeZ#O/#R",
     "Bandpass Bessel filter, matched z-transform, order #O, -3dB frequencies #R" },
   { des_bsbez, "BsBeZ#O/#R",
     "Bandstop Bessel filter, matched z-transform, order #O, -3dB frequencies #R" },
   { des_lpbuz, "LpBuZ#O/#F",
     "Lowpass Butterworth filter, matched z-transform, order #O, -3dB frequency #F" },
   { des_hpbuz, "HpBuZ#O/#F",
     "Highpass Butterworth filter, matched z-transform, order #O, -3dB frequency #F" },
   { des_bpbuz, "BpBuZ#O/#R",
     "Bandpass Butterworth filter, matched z-transform, order #O, -3dB frequencies #R" },
   { des_bsbuz, "BsBuZ#O/#R",
     "Bandstop Butterworth filter, matched z-transform, order #O, -3dB frequencies #R" },
   { des_lpchz, "LpChZ#O/#V/#F",
     "Lowpass Chebyshev filter, matched z-transform, order #O, "
     "passband ripple #VdB, -3dB frequency #F" },
   ...
   { des_bpchz, "BpChZ#O/#V/#R",
     ...
     "passband ripple #VdB, -3dB frequencies #R" },
   { des_bschz, "BsChZ#O/#V/#R",
     "Bandstop Chebyshev filter, matched z-transform, order #O, "
     "passband ripple #VdB, -3dB frequencies #R" },
   { des_lpbq, "LpBq#O/#V/#F",
     "Lowpass biquad filter, order #O, Q=#V, -3dB frequency #F" },
   { des_hpbq, "HpBq#O/#V/#F",
     "Highpass biquad filter, order #O, Q=#V, -3dB frequency #F" },
   { des_bpbq, "BpBq#O/#V/#F",
   ...
   { des_lpbube, "LpBuBe#O/#V/#F",
     "Lowpass Butterworth-Bessel #V% cross, order #O, -3dB frequency #F" },
   { des_lpbl, "LpBl/#F",
     "Lowpass Blackman window, -3dB frequency #F" },
   { des_lphm, "LpHm",
     "Lowpass Hamming window @@@" },
   { des_lphn, "LpHn",
     "Lowpass Hanning window @@@" },
   { 0, 0, 0 }
};
...
//	are adjusted automatically to get true 50% values at the
...
//
//	Auto-adjust input frequency to give correct 50% point to 6 figures
//
...
#define TEST(aa) { if (rv) {free(rv);rv= 0;} rv= DESIGN(aa); resp= fid_response(rv, f0); }
   ...
   while (1) {
      ...
      TEST(a1);
      if (resp >= 0.4999995 && resp < 0.5000005) break;
      if (incr == (resp > 0.5))
	 a2= a1;
      else 
	 a0= a1;
...
#define DESIGN(mm,ww) { if (rv) {free(rv);rv= 0;} \
   ...
   err0= fabs(0.5-r0); err1= fabs(0.5-r1); cnt_design++; }
...
//
//	Expand a specification string to the given output stream
//

static void 
expand_spec(FILE *out, char *str) {
   int ch;
   char *p;
   ...
   while ((ch= *str++)) {
      if (ch == '#') {
	 switch (*str++) {
	  case 'O': p= "<order>"; break;
	  case 'F': p= "<freq>"; break;
	  case 'R': p= "<range>"; break;
	  case 'V': p= "<value>"; break;
	  default: p= 0; fprintf(out, "<%c>", str[-1]); break;
	 }
	 fprintf(out, "%s", p);
      } else {
	 fputc(ch, out);
...
fid_list_filters(FILE *out) {
   ...
   for (a= 0; filter[a].fmt; a++) {
      expand_spec(out, filter[a].fmt);
      fprintf(out, "\n    ");
      expand_spec(out, filter[a].txt);
      fprintf(out, "\n");
   }
}
...
#ifdef RF_COMBINED
#include "rf_combined.c"
#endif

#ifdef RF_CMDLIST
#include "rf_cmdlist.c"
#endif
